# 📌 Phase 2: Core Python (Weeks 5–8)

> Python's built-in data structures — the tools you will use every single day.
>

**Modules 5–8** · 4 modules + projects

| # | File | Contents |
| --- | --- | --- |
| 1 | [Module 5: Strings](module-05-strings.md) | Creation, indexing & slicing, immutability, methods, f-strings, escape & raw strings |
| 2 | [Module 6: Lists](module-06-lists.md) | Creation, indexing, methods, list comprehension, nested lists, unpacking, copying |
| 3 | [Module 7: Tuples and Sets](module-07-tuples-and-sets.md) | Packing/unpacking, immutability, set operations, frozenset, set comprehension |
| 4 | [Module 8: Dictionaries](module-08-dictionaries.md) | Creation, methods, nested dicts, iteration, dict comprehension, dictionary vs JSON |
| — | [Projects & Key Takeaways](projects-and-takeaways.md) | 8 practice projects and the phase summary |
| — | [**60 Practice Questions**](questions.md) | Strings, lists, tuples, sets and dictionaries — including manual implementations of things the built-ins do for you. Difficulty rises from Q1 to Q60. |
| ✅ | [**Automated Judge**](tests/README.md) | Write your answer, run `python run_tests.py 4 --diff`, get PASS/FAIL. **116 test cases across 60 questions.** |
| 🔑 | [**Solutions**](solutions.md) | A worked solution for every graded question, each verified against the full suite. Write yours first. |
| 📋 | [**5 Assignments**](assignments.md) | Text analysis engine, sorting lab, grade management, contact book, conversion toolkit |

---

[← Phase 1: Foundation](../phase-1-foundation/README.md) · [Phase 3: Advanced Python →](../phase-3-advanced-python/README.md)
